...
$( function() {
	...
	tab.add( {
		...
		<%if(outPatientId != null) {%>
			,url :"PatMasterTimer/toOutPatientTimer.do?outPatientId=<%=outPatientId%>&PatientId=<%=patient_id%>"
		<%} else if(inPatientId != null) {%>
			,url :"PatMasterTimer/toInPatientTimer.do?inPatientId=<%=inPatientId%>&PatientId=<%=patient_id%>"
		<%} else {%>
			,url :"PatMasterTimer/toPatientTimer.do?PatientId=<%=patient_id%>"
		<%}%>
		,isClosed :false
	});
	/**tab.add( {
