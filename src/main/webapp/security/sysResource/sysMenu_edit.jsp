...
<body>
	<form  action="sysResource/dnns_editMenu.do" name="menuForm" id="menuForm" method="post" >
		<span>模块名称：${modul.resourceName }</span>
		<input type="hidden" name="resourceId" value="${sysResource.resourceId }">
		<div id="zhongxin">
		<table>
			<tr>
				<td><input type="text" name="resourceName" id="resourceName" placeholder="这里输入菜单名称" value="${sysResource.resourceName }" title="菜单名称"/></td>
			</tr>
			<tr>
				<td><input type="text" name="resourceDescription" id="resourceDescription" placeholder="这里输入菜单描述" value="${sysResource.resourceDescription }" title="菜单描述"/></td>
			</tr>
			<tr>
